...
public: // methods
	...
	// Returns INT_MAX if goal is not defined or there is no solution
	int GetSolutionCost()
	{
...
public:
	virtual ~AStarState() {}
	virtual int GoalDistanceEstimate( T &nodeGoal ) = 0; // Heuristic function which computes the estimated cost to the goal node
	...
	virtual int GetCost( T &successor ) = 0; // Computes the cost of travelling from this node to the successor node
